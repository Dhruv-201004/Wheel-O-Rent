...
const CarDetails = () => {
  ...
  // Fetch car details from context or API
  useEffect(() => {
    const fetchCarDetails = async () => {
      try {
        // First, try to find car in context
        const contextCar = cars.find((car) => car._id === id);
        if (contextCar) {
          setCar(contextCar);
          setIsLoading(false);
          return;
        ...
        if (data.success) {
          const foundCar = data.cars.find((car) => car._id === id);
          if (foundCar) {
            setCar(foundCar);
          } else {
    ...
    <div className="!px-16 md:!px-16 lg:!px-24 xl:!px-32 !mt-16 ">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
        ...
        >
          ...
          <p className="text-sm text-center">Secure payment with Stripe</p>
        </motion.form>
